...
class GDCNN_AR_AMPO(nn.Module):
    def __init__(self, batch_size=64, window=32, n_zone=20, n_kernels=32,drop_prob=0.1,learning_rate=0.005,
        ...
        super(GDCNN_AR_AMPO, self).__init__()
    ...
    def forward(self, x):
        ''' Nonlinear component'''
        # Dual CNN
        gCNN_output, *_ = self.gCNN(x)  # [512,5,32]
        lCNN_output, *_ = self.lCNN(x)  # [512,5,32]
        # gated mechanisim
...
for i in range(len(year)):
    for j in range(len(zone_set)):
        for k in range(len(lag_set)):
            ...
            np.random.seed(101)
            model = GDCNN_AR_AMPO(batch_size=batch_size, n_zone=zone_set[j], window=24, lag=lag_set[k])
